    ...
    >({
      queryFn: async (args, __, _, baseQuery) => {
        ...
        for (let { id, mediaType } of currentUserList) {
          ...
          if (mediaType === MediaType.Movie) {
            output.push(new MovieListItem(IMG_BASE_URL_300, movieData, mediaType).getValues());
          } else if (mediaType === MediaType.TV) {
            output.push(new TVListItem(IMG_BASE_URL_300, movieData, mediaType).getValues());
          }
        }
        ...
        if (output.length === 0) {
          return {
            error: {
              status: 400,
              data: {
                message: `${lang === 'ru' ? 'Этот список пуст' : 'This list is empty'}`,
                status: 400,
              }
            } as FetchBaseQueryError
          }
